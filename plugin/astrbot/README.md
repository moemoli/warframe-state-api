...
- **市场**：WM 实时价格、紫卡拍卖全语法筛选（词条/洗数/等级/段位/价格/极性，由
  `/api/wfm/auctions` **服务端过滤**并回传命中 `total` 与逐条 `matched_conditions`）、
  词条价差、价格趋势（48h/90d 真实成交）、杜卡德垃圾分档、赤毒/信条武器库
- **订阅**：`蹲钢月`、`蹲赛中`、`蹲三傻`、`蹲奸商` 等口语简称 → 命中主动推送并 @ 订阅者；
  同一条命中会把同会话内所有订阅者合并到一条消息并全部 @；
  支持组合解析（`蹲钢铁赛中`=钢铁赛德娜中断裂缝、`蹲九重天生存`=虚空风暴生存）、
  循环订阅（`蹲地球 白天`/`蹲金星 寒冷`/`蹲火卫二 Vome`/`蹲双衍 愤怒`）、仲裁订阅（`蹲仲裁 欧罗巴`）
